...
export default async function CaseDetail({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  const genlayerExplorer = process.env.STUDIO_NET_EXPLORER_URL || "https://explorer-studio.genlayer.com";
  const passed = item.verdict.criteria.filter((criterion) => criterion.decision === "PASS").length;
  const partial = item.verdict.criteria.filter((criterion) => criterion.decision === "PARTIAL").length;
  const failed = item.verdict.criteria.filter((criterion) => criterion.decision === "FAIL" || criterion.decision === "UNVERIFIABLE").length;
  return <><Nav /><main className="shell case-page">
    ...
    <div className="case-layout">
      <div className="case-main">
        <section className="case-section"><div className="section-title"><ShieldCheck size={18}/><div><span>Public adjudication</span><h2>Why GenLayer reached this result</h2></div></div><p className="final-rationale">{item.verdict.final_rationale || "The finalized contract result contains no additional public rationale."}</p><div className="verdict-breakdown"><div className="verdict-breakdown-chart" style={{ background: `conic-gradient(var(--green) 0 ${passed / Math.max(item.verdict.criteria.length, 1) * 100}%, #f2c66d ${passed / Math.max(item.verdict.criteria.length, 1) * 100}% ${(passed + partial) / Math.max(item.verdict.criteria.length, 1) * 100}%, #ff7f87 ${(passed + partial) / Math.max(item.verdict.criteria.length, 1) * 100}% 100%)` }}><strong>{item.verdict.criteria.length}</strong><span>criteria</span></div><div className="verdict-breakdown-legend"><span><i className="pass" /> {passed} passed</span><span><i className="partial" /> {partial} partial</span><span><i className="fail" /> {failed} needs attention</span></div></div>{item.verdict.critical_failures.length > 0 && <div className="finding-alert"><strong>Critical failures</strong><p>{item.verdict.critical_failures.join(", ")}</p></div>}{item.verdict.missing_evidence.length > 0 && <div className="finding-alert neutral"><strong>Missing evidence</strong><p>{item.verdict.missing_evidence.join(" · ")}</p></div>}</section>
