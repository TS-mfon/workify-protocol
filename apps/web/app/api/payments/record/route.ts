...
export async function POST(request: Request) {
  try {
    ...
    const key = `${input.jobId.toLowerCase()}:${input.network}:${input.kind}:${input.kind === "verification" ? input.attempt : "appeal"}`;
...
export async function GET(request: Request) {
  try {
    const input = schema.pick({ jobId: true, kind: true, attempt: true, network: true }).parse({
