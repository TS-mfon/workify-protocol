...
export function LiveJobDetail({ jobId }: { jobId: string }) {
  ...
  const { job } = detail;
  const nextHref = detail.status === "AWAITING_DELIVERY" ? `/app/jobs/${jobId}/deliver` : detail.status === "DELIVERY_LOCKED" ? `/app/jobs/${jobId}/verify` : detail.status === "APPEAL_WINDOW" ? `/app/jobs/${jobId}/appeal` : `/explorer/${jobId}`;
  const nextLabel = detail.status === "AWAITING_DELIVERY" ? "Submit and lock delivery" : detail.status === "DELIVERY_LOCKED" ? "Start GenLayer review" : detail.status === "APPEAL_WINDOW" ? "Review verdict or appeal" : detail.status === "SETTLEABLE" ? "Release escrow funds" : detail.status === "SETTLED" || detail.status === "REFUNDED" ? "View public record" : "Track contract state";
  const nextText = detail.status === "AWAITING_DELIVERY" ? "The assigned worker must submit and lock a public evidence manifest." : detail.status === "DELIVERY_LOCKED" ? "The client starts one direct StudioNet V12 review after evidence is locked." : detail.status === "APPEAL_WINDOW" ? "The verdict is final; either party may appeal during the five-minute window." : detail.status === "SETTLEABLE" ? "The escrow is eligible for permissionless payout or refund." : detail.status === "SETTLED" ? "Funds have been released according to the finalized payout." : detail.status === "REFUNDED" ? "The client refund has closed this contract." : detail.status.replaceAll("_", " ");
  return <div className="job-detail-live">
    ...
    <div className="job-detail-next"><strong>Next step</strong><p>{nextText}</p><Link className="button" href={nextHref}>{nextLabel} <ArrowUpRight size={15}/></Link></div>
  </div>;
}
