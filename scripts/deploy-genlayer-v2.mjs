...
const version = Number(process.env.WORKIFY_CONTRACT_VERSION ?? 2);
if (![2, 3].includes(version)) throw new Error("WORKIFY_CONTRACT_VERSION must be 2 or 3");
...
let manifest;
try { manifest = JSON.parse(await readFile(manifestUrl, "utf8")); } catch { manifest = { network: "testnet-bradbury", version, operator: account.address, verifiers: {} }; }
const policies = [
...
const requested = new Set((process.env.WORKIFY_V2_POLICIES ?? "github").split(","));
...
for (const [name, workType, policyVersion] of policies) {
  ...
  if (receipt.resultName !== "AGREE" || receipt.txExecutionResultName !== "FINISHED_WITH_RETURN") throw new Error(`${name} v2 deployment failed`);
