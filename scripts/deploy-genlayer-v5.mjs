...
const requested = new Set((process.env.WORKIFY_V5_POLICIES ?? "github").split(","));
...
for (const [name, workType, policyVersion] of policies) {
  ...
  if (receipt.resultName !== "AGREE" || receipt.txExecutionResultName !== "FINISHED_WITH_RETURN") throw new Error(`${name} v5 deployment failed`);
