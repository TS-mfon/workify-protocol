...
const treasury = process.env.NEXT_PUBLIC_GENLAYER_TREASURY_ADDRESS;
if (!treasury) throw new Error("NEXT_PUBLIC_GENLAYER_TREASURY_ADDRESS is required");
...
const requested = new Set((process.env.WORKIFY_V8_POLICIES ?? "github").split(","));
...
for (const [name, workType, policyVersion] of policies) {
  ...
  let hash = manifest.verifiers[name]?.hash;
  if (hash) {
    try {
      const receipt = await client.waitForTransactionReceipt({ hash, status: TransactionStatus.ACCEPTED, interval: 5_000, retries: 12 });
      if (receipt.resultName === "AGREE" && receipt.txExecutionResultName === "FINISHED_WITH_RETURN") {
  ...
  for (let retry = 0; retry < 8; retry += 1) {
    try { hash = await client.deployContract({ code, args: [account.address, treasury, workType, policyVersion] }); break; }
  ...
  if (receipt.resultName !== "AGREE" || receipt.txExecutionResultName !== "FINISHED_WITH_RETURN") throw new Error(`${name} v8 deployment failed`);
  manifest.verifiers[name] = { address: receipt.txDataDecoded?.contractAddress, hash, status: receipt.statusName, policyVersion };
