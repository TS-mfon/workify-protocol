...
contract WorkEscrowV1 is Ownable2Step, Pausable, ReentrancyGuard, EIP712 {
    ...
    event AttemptUndetermined(bytes32 indexed jobId, uint8 attempt, bool appeal, uint64 retryDeadline);
    ...
    ) Ownable(initialOwner) EIP712("Workify", "1") {
        if (
    ...
    function importFinalVerdict(VerdictAttestation calldata verdict, bytes calldata signature) external {
        ...
        if (verdict.appeal) {
            ...
            _applyFinalDecision(verdict.jobId, job, verdict.decision, verdict.payoutBps, verdict.resultHash, true);
            return;
        }
        ...
        _applyFinalDecision(verdict.jobId, job, verdict.decision, verdict.payoutBps, verdict.resultHash, false);
    }
    ...
    function recordAttemptOutcome(OutcomeAttestation calldata outcome, bytes calldata signature) external {
        ...
        if (outcome.appeal) {
            ...
            if (job.appealAttempts == MAX_ATTEMPTS) {
                ...
                emit AttemptUndetermined(outcome.jobId, outcome.attempt, true, job.retryDeadline);
            }
            return;
        }
        ...
        if (job.attempts == MAX_ATTEMPTS) {
            ...
            job.retryDeadline = uint64(block.timestamp + RETRY_WINDOW);
            emit AttemptUndetermined(outcome.jobId, outcome.attempt, false, job.retryDeadline);
        }
    }
    ...
    function _applyFinalDecision(
        ...
        emit VerdictImported(jobId, decision, payoutBps, resultHash, appeal);
    }
