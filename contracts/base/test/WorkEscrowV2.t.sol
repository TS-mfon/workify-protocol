...
contract WorkEscrowV2Test is Test {
    ...
    WorkEscrowV2 private escrow;

    function setUp() external {
        attestor = vm.addr(attestorKey);
        usdc = new MockUSDC();
        treasury = new BaseTreasuryV2(owner);
        escrow = new WorkEscrowV2(address(usdc), address(treasury), owner, attestor, operator);
    ...
    function testCreateJobLocksFundsAtomically() external {
        ...
        WorkEscrowV2.Job memory job = escrow.getJob(jobId);
    ...
    function testPassPaysWorkerAndTreasury() external {
        ...
        _importVerdict(jobId, WorkEscrowV2.Decision.PASS, 10_000, 1, false, 1);
    ...
    function testRefundsClientWithoutFeeForRejectedVerdict() external {
        ...
        bytes32 jobId = _lockedJob(100e6);
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV2.Decision.FAIL, 0, 1, false, 2);
        vm.warp(block.timestamp + 5 minutes + 1);
        escrow.settle(jobId);
    ...
    function testPartialConservesEscrowAndChargesOnlyWorkerShare() external {
        ...
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV2.Decision.PARTIAL, 4_000, 1, false, 3);
        vm.warp(block.timestamp + 5 minutes + 1);
        escrow.settle(jobId);
    ...
    function testSecondUndeterminedAttemptSettlesFiftyFifty() external {
        ...
        WorkEscrowV2.Job memory job = escrow.getJob(jobId);
        assertEq(uint8(job.decision), uint8(WorkEscrowV2.Decision.SPLIT));
    }
    ...
    function testWorkerCanOpenAppealAndFreezeSettlement() external {
        bytes32 jobId = _lockedJob(100e6);
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV2.Decision.FAIL, 0, 1, false, 10);
        vm.prank(worker);
    ...
    function testAttestationReplayIsRejected() external {
        bytes32 jobId = _lockedJob(100e6);
        _request(false, jobId);
        WorkEscrowV2.VerdictAttestation memory verdict =
            _verdict(jobId, WorkEscrowV2.Decision.PASS, 10_000, 1, false, 55);
        bytes memory signature = _signVerdict(verdict);
        escrow.importFinalVerdict(verdict, signature);
        vm.expectRevert(WorkEscrowV2.Replay.selector);
        escrow.importFinalVerdict(verdict, signature);
    }
    ...
    function testForgedAttestationCannotImportVerdict() external {
        ...
        _request(false, jobId);
        WorkEscrowV2.VerdictAttestation memory verdict =
            _verdict(jobId, WorkEscrowV2.Decision.PASS, 10_000, 1, false, 56);
        bytes32 digest = keccak256("forged verdict");
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(0xBAD, digest);
        vm.expectRevert(WorkEscrowV2.InvalidSignature.selector);
    ...
    function testVerdictCannotSubstituteLockedHashes() external {
        ...
        _request(false, jobId);
        WorkEscrowV2.VerdictAttestation memory verdict =
            _verdict(jobId, WorkEscrowV2.Decision.PASS, 10_000, 1, false, 57);
        verdict.evidenceHash = keccak256("attacker evidence");
        vm.expectRevert(WorkEscrowV2.InvalidEvidence.selector);
    ...
    function testPermissionlessSettlementCannotRedirectRecipients() external {
        ...
        bytes32 jobId = _lockedJob(100e6);
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV2.Decision.PARTIAL, 4_000, 1, false, 58);
        vm.warp(block.timestamp + 5 minutes + 1);
    ...
    function testFuzzRewardConservation(uint128 reward, uint16 payoutBps) external {
        ...
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV2.Decision.PARTIAL, payoutBps, 1, false, 77);
        vm.warp(block.timestamp + 5 minutes + 1);
    ...
    function _verdict(
        bytes32 jobId,
        WorkEscrowV2.Decision decision,
        uint16 payoutBps,
        uint8 attempt,
        bool appeal,
        uint256 nonce
    ) private view returns (WorkEscrowV2.VerdictAttestation memory) {
        return WorkEscrowV2.VerdictAttestation({
            jobId: jobId,
    ...
    function _recordUndetermined(bytes32 jobId, uint8 attempt, bool appeal, uint256 nonce) private {
        WorkEscrowV2.OutcomeAttestation memory outcome = WorkEscrowV2.OutcomeAttestation({
            jobId: jobId,
            verifierId: verifierId,
            ...
            evidenceHash: evidenceHash,
            policyHash: policyHash,
            outcome: WorkEscrowV2.AttemptOutcome.UNDETERMINED,
            nonce: nonce,
            appeal: appeal
        });
    ...
    function _signVerdict(WorkEscrowV2.VerdictAttestation memory verdict)
        private
        view
