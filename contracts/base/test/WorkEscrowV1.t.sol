...
contract WorkEscrowV1Test is Test {
    ...
    BaseTreasuryV1 private treasury;
    WorkEscrowV1 private escrow;

    function setUp() external {
        attestor = vm.addr(attestorKey);
        usdc = new MockUSDC();
        treasury = new BaseTreasuryV1(owner);
        escrow = new WorkEscrowV1(address(usdc), address(treasury), owner, attestor, operator);
    ...
    function testCreateJobLocksFundsAtomically() external {
        ...
        WorkEscrowV1.Job memory job = escrow.getJob(jobId);
    ...
    function testPassPaysWorkerAndTreasury() external {
        ...
        _importVerdict(jobId, WorkEscrowV1.Decision.PASS, 10_000, 1, false, 1);
    ...
    function testRefundsClientWithoutFeeForRejectedVerdict() external {
        ...
        bytes32 jobId = _lockedJob(100e6);
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV1.Decision.FAIL, 0, 1, false, 2);
        vm.warp(block.timestamp + 5 minutes + 1);
        escrow.settle(jobId);
    ...
    function testPartialConservesEscrowAndChargesOnlyWorkerShare() external {
        ...
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV1.Decision.PARTIAL, 4_000, 1, false, 3);
        vm.warp(block.timestamp + 5 minutes + 1);
        escrow.settle(jobId);
    ...
    function testThreeUndeterminedAttemptsSettleFiftyFifty() external {
        bytes32 jobId = _lockedJob(100e6);
        ...
        WorkEscrowV1.Job memory job = escrow.getJob(jobId);
        assertEq(uint8(job.decision), uint8(WorkEscrowV1.Decision.SPLIT));
    }
    ...
    function testWorkerCanOpenAppealAndFreezeSettlement() external {
        bytes32 jobId = _lockedJob(100e6);
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV1.Decision.FAIL, 0, 1, false, 10);
        vm.prank(worker);
    ...
    function testAttestationReplayIsRejected() external {
        ...
        WorkEscrowV1.VerdictAttestation memory verdict = _verdict(
            jobId, WorkEscrowV1.Decision.PASS, 10_000, 1, false, 55
        );
        bytes memory signature = _signVerdict(verdict);
        escrow.importFinalVerdict(verdict, signature);
        vm.expectRevert(WorkEscrowV1.Replay.selector);
        escrow.importFinalVerdict(verdict, signature);
    }
    ...
    function testFuzzRewardConservation(uint128 reward, uint16 payoutBps) external {
        ...
        _request(false, jobId);
        _importVerdict(jobId, WorkEscrowV1.Decision.PARTIAL, payoutBps, 1, false, 77);
        vm.warp(block.timestamp + 5 minutes + 1);
    ...
    function _verdict(
        bytes32 jobId,
        WorkEscrowV1.Decision decision,
        uint16 payoutBps,
        uint8 attempt,
        bool appeal,
        uint256 nonce
    ) private view returns (WorkEscrowV1.VerdictAttestation memory) {
        return WorkEscrowV1.VerdictAttestation({
            jobId: jobId,
    ...
    function _importVerdict(
        ...
        WorkEscrowV1.VerdictAttestation memory verdict = _verdict(
            jobId, decision, payoutBps, attempt, appeal, nonce
        );
        escrow.importFinalVerdict(verdict, _signVerdict(verdict));
    }

    function _recordUndetermined(bytes32 jobId, uint8 attempt, bool appeal, uint256 nonce) private {
        WorkEscrowV1.OutcomeAttestation memory outcome = WorkEscrowV1.OutcomeAttestation({
            jobId: jobId,
            verifierId: verifierId,
            ...
            evidenceHash: evidenceHash,
            policyHash: policyHash,
            outcome: WorkEscrowV1.AttemptOutcome.UNDETERMINED,
            nonce: nonce,
            appeal: appeal
        });
    ...
    function _signVerdict(WorkEscrowV1.VerdictAttestation memory verdict) private view returns (bytes memory) {
        bytes32 structHash = keccak256(
            abi.encode(
                VERDICT_TYPEHASH,
                verdict.jobId,
