...
contract WorkEscrowV4 is Ownable2Step, Pausable, ReentrancyGuard, EIP712 {
    ...
    event AppealResolved(bytes32 indexed jobId, Decision decision, uint16 payoutBps, bytes32 resultHash);
    ...
    ) Ownable(initialOwner) EIP712("Workify", "3") {
        if (
    ...
    function _applyFinalDecision(
        ...
        if (appeal) {
            job.status = Status.SETTLEABLE;
            emit AppealResolved(jobId, decision, payoutBps, resultHash);
        } else {
