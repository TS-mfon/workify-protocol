...
class WorkVerifierV2(gl.Contract):
    operator: Address
    work_type: str
    policy_version: str
    ...
    def verify(
        ...
        WorkVerifierV2._validate_request(None,
        ...
        def leader_fn():
            return WorkVerifierV2._evaluate(None,
        ...
        def validator_fn(leader_result) -> bool:
            ...
            try:
                ...
                return (
                    leader["decision"] == independent["decision"]
                    and leader["payout_bps"] == independent["payout_bps"]
                    and abs(int(leader["score"]) - int(independent["score"])) <= 15
                    and WorkVerifierV2._criterion_map(None, leader)
                    == WorkVerifierV2._criterion_map(None, independent)
                )
            except Exception:
                return False
        ...
        key = WorkVerifierV2._key(None, job_id, attempt, appeal)
    ...
    def get_verdict(self, job_id: str, attempt: u32, appeal: bool) -> str:
        return self.verdicts.get(WorkVerifierV2._key(None, job_id, attempt, appeal), "")
    ...
    def _evaluate(self, work_type: str, policy_version: str, job_id: str, specification_url: str, specification_hash: str, evidence_url: str, evidence_hash: str, attempt: u32, appeal: bool, appeal_context_url: str) -> str:
        specification_bytes = WorkVerifierV2._fetch(None, specification_url, MAX_DOCUMENT_BYTES)
        evidence_bytes = WorkVerifierV2._fetch(None, evidence_url, MAX_DOCUMENT_BYTES)
        ...
        sources = WorkVerifierV2._fetch_sources(None, evidence)
        appeal_context = ""
        if appeal:
            appeal_context = WorkVerifierV2._fetch(None, appeal_context_url, MAX_DOCUMENT_BYTES).decode("utf-8", errors="replace")
        prompt = WorkVerifierV2._prompt(None, work_type, policy_version, job_id, specification, evidence, sources, attempt, appeal, appeal_context)
        raw = gl.nondet.exec_prompt(prompt, response_format="json")
        return WorkVerifierV2._normalize(None, raw, policy_version, specification_hash, evidence_hash, attempt)
    ...
    def _fetch_sources(self, evidence: dict) -> list:
        ...
        for artifact in artifacts[:MAX_SOURCES]:
            ...
            if not url.startswith("https://"):
                continue
            body = WorkVerifierV2._fetch(None, url, MAX_SOURCE_BYTES)
...
ATTEMPT: {attempt}
APPEAL: {appeal}
DOMAIN RUBRIC: {WorkVerifierV2._rubric(None, work_type)}
