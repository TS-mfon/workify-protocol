...
class WorkVerifierV12(gl.Contract):
    operator: Address
    treasury: Address
    work_type: str
    policy_version: str
    ...
    def __init__(self, operator: str, treasury: str, work_type: str, policy_version: str, verification_fee: u256, appeal_fee: u256):
        ...
        if verification_fee != VERIFICATION_FEE:
            raise gl.vm.UserError("[EXPECTED] Invalid verification fee")
        if appeal_fee != APPEAL_FEE:
    ...
    def verify(
        ...
        def specification_fn():
            return WorkVerifierV12._load_specification_text(None, specification_url, specification_hash)

        specification_text = gl.eq_principle.strict_eq(specification_fn)
        specification = json.loads(specification_text)

        def evidence_fn():
            adjudication_input = WorkVerifierV12._build_input(None,
...
Keep each rationale factual, public, and under 800 characters. Do not reveal private chain-of-thought.
            Policy rubric: """ + WorkVerifierV12._rubric(None, work_type) + "\nADJUDICATION INPUT:\n" + adjudication_input
        ...
        parsed = WorkVerifierV12._normalize(None, raw_verdict, specification, policy_version, specification_hash, evidence_hash, attempt, appeal)
    ...
    def get_verdict(self, job_id: str, attempt: u32, appeal: bool) -> str:
        return self.verdicts.get(WorkVerifierV12._key(None, job_id, attempt, appeal), "")
    ...
    def _load_specification_text(self, specification_url: str, specification_hash: str) -> str:
        specification_bytes = WorkVerifierV12._fetch(None, specification_url, MAX_DOCUMENT_BYTES)
    ...
    def _build_input(self, work_type: str, policy_version: str, job_id: str, specification: dict, evidence_url: str, evidence_hash: str, attempt: u32, appeal: bool, appeal_context_url: str) -> str:
        evidence_bytes = WorkVerifierV12._fetch(None, evidence_url, MAX_DOCUMENT_BYTES)
        ...
        sources = WorkVerifierV12._fetch_sources(None, evidence)
        appeal_context = ""
        if appeal:
            appeal_context = WorkVerifierV12._fetch(None, appeal_context_url, MAX_DOCUMENT_BYTES).decode("utf-8", errors="replace")
        return WorkVerifierV12._prompt(None, work_type, policy_version, job_id, specification, sources, attempt, appeal, appeal_context)
    ...
    def _fetch_sources(self, evidence: dict) -> list:
        ...
        for artifact in artifacts[:MAX_SOURCES]:
            ...
            if not url.startswith("https://"):
                continue
            body = WorkVerifierV12._fetch(None, url, MAX_SOURCE_BYTES)
    ...
    def _normalize(self, raw, specification: dict, policy_version: str, specification_hash: str, evidence_hash: str, attempt: u32, appeal: bool) -> dict:
        value = raw if isinstance(raw, dict) else json.loads(WorkVerifierV12._json_text(None, str(raw)))
        ...
        normalized = {"decision": decision, "payout_bps": payout_bps, "score": payout_bps // 100, "confidence": WorkVerifierV12._confidence(None, value.get("confidence", 0)), "criteria": normalized_criteria, "critical_failures": [item["id"] for item in normalized_criteria if item["critical"] and item["decision"] != "PASS"], "missing_evidence": [str(item)[:300] for item in list(value.get("missing_evidence", []))[:20]], "evidence_root": evidence_hash.lower(), "specification_hash": specification_hash.lower(), "policy_version": policy_version, "attempt": int(attempt), "appeal": appeal, "final_rationale": str(value.get("final_rationale", value.get("final_reasoning", "")))[:2000], "verdict_available": True, "execution": "FINISHED_WITH_RETURN", "consensus_requirement": "COMPARATIVE"}
