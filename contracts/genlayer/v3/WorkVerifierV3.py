...
class WorkVerifierV3(gl.Contract):
    operator: Address
    work_type: str
    policy_version: str
    ...
    def verify(
        ...
        WorkVerifierV3._validate_request(None,
        ...
        def leader_fn():
            return WorkVerifierV3._evaluate(None,
        ...
        verdict = gl.eq_principle.prompt_comparative(
            leader_fn,
            principle="The decision and payout_bps fields must be exactly identical. Each acceptance criterion id must have the same decision. Critical failures and missing evidence must not contradict each other. Scores and prose may differ when the substantive adjudication is the same.",
        )
        parsed = json.loads(verdict)
        key = WorkVerifierV3._key(None, job_id, attempt, appeal)
    ...
    def get_verdict(self, job_id: str, attempt: u32, appeal: bool) -> str:
        return self.verdicts.get(WorkVerifierV3._key(None, job_id, attempt, appeal), "")
    ...
    def _evaluate(self, work_type: str, policy_version: str, job_id: str, specification_url: str, specification_hash: str, evidence_url: str, evidence_hash: str, attempt: u32, appeal: bool, appeal_context_url: str) -> str:
        specification_bytes = WorkVerifierV3._fetch(None, specification_url, MAX_DOCUMENT_BYTES)
        evidence_bytes = WorkVerifierV3._fetch(None, evidence_url, MAX_DOCUMENT_BYTES)
        ...
        sources = WorkVerifierV3._fetch_sources(None, evidence)
        appeal_context = ""
        if appeal:
            appeal_context = WorkVerifierV3._fetch(None, appeal_context_url, MAX_DOCUMENT_BYTES).decode("utf-8", errors="replace")
        prompt = WorkVerifierV3._prompt(None, work_type, policy_version, job_id, specification, evidence, sources, attempt, appeal, appeal_context)
        raw = gl.nondet.exec_prompt(prompt, response_format="json")
        return WorkVerifierV3._normalize(None, raw, policy_version, specification_hash, evidence_hash, attempt)
    ...
    def _fetch_sources(self, evidence: dict) -> list:
        ...
        for artifact in artifacts[:MAX_SOURCES]:
            ...
            if not url.startswith("https://"):
                continue
            body = WorkVerifierV3._fetch(None, url, MAX_SOURCE_BYTES)
...
ATTEMPT: {attempt}
APPEAL: {appeal}
DOMAIN RUBRIC: {WorkVerifierV3._rubric(None, work_type)}
...
Return only JSON with decision, payout_bps, score, confidence, criteria, critical_failures,
missing_evidence, and final_reasoning. PASS requires payout_bps 10000. FAIL and
UNVERIFIABLE require 0. PARTIAL requires 1-9999. Every criterion must appear once.
"""
    ...
    def _normalize(self, raw, policy_version: str, specification_hash: str, evidence_hash: str, attempt: u32) -> str:
        ...
        normalized = {"decision": decision, "payout_bps": payout_bps, "score": max(0, min(100, int(value.get("score", 0)))), "confidence": max(0, min(100, int(value.get("confidence", 0)))), "criteria": criteria, "critical_failures": list(value.get("critical_failures", []))[:20], "missing_evidence": list(value.get("missing_evidence", []))[:20], "evidence_root": evidence_hash.lower(), "specification_hash": specification_hash.lower(), "policy_version": policy_version, "attempt": int(attempt), "final_reasoning": str(value.get("final_reasoning", ""))[:4000]}
