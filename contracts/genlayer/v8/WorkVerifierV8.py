...
class WorkVerifierV8(gl.Contract):
    operator: Address
    treasury: Address
    work_type: str
    policy_version: str
...
Policy rubric: """ + WorkVerifierV8._rubric(None, work_type) + "\nADJUDICATION INPUT:\n" + adjudication_input
        ...
        parsed = WorkVerifierV8._normalize(None, raw_verdict, specification, policy_version, specification_hash, evidence_hash, attempt, appeal)
        verdict = json.dumps(parsed, sort_keys=True, separators=(",", ":"))
        key = WorkVerifierV8._key(None, job_id, attempt, appeal)
    ...
    def get_verdict(self, job_id: str, attempt: u32, appeal: bool) -> str:
        return self.verdicts.get(WorkVerifierV8._key(None, job_id, attempt, appeal), "")
    ...
    def _require_fee(self, treasury_address: str, job_id: str, attempt: u32, appeal: bool, fee_payer: str) -> None:
        ...
        expected = u256(1000000000000000000) if appeal else u256(100000000000000000)
        if payment["amount"] != expected:
            raise gl.vm.UserError("[EXPECTED] Verification fee not finalized")
        if str(payment["payer"]).lower() != str(payer).lower():
    ...
    def _load_specification_text(self, specification_url: str, specification_hash: str) -> str:
        specification_bytes = WorkVerifierV8._fetch(None, specification_url, MAX_DOCUMENT_BYTES)
    ...
    def _build_input(self, work_type: str, policy_version: str, job_id: str, specification: dict, evidence_url: str, evidence_hash: str, attempt: u32, appeal: bool, appeal_context_url: str) -> str:
        evidence_bytes = WorkVerifierV8._fetch(None, evidence_url, MAX_DOCUMENT_BYTES)
        ...
        sources = WorkVerifierV8._fetch_sources(None, evidence)
        appeal_context = ""
        if appeal:
            appeal_context = WorkVerifierV8._fetch(None, appeal_context_url, MAX_DOCUMENT_BYTES).decode("utf-8", errors="replace")
        return WorkVerifierV8._prompt(None, work_type, policy_version, job_id, specification, sources, attempt, appeal, appeal_context)
    ...
    def _fetch_sources(self, evidence: dict) -> list:
        ...
        for artifact in artifacts[:MAX_SOURCES]:
            ...
            if not url.startswith("https://"):
                continue
            body = WorkVerifierV8._fetch(None, url, MAX_SOURCE_BYTES)
    ...
    def _normalize(self, raw, specification: dict, policy_version: str, specification_hash: str, evidence_hash: str, attempt: u32, appeal: bool) -> dict:
        value = raw if isinstance(raw, dict) else json.loads(WorkVerifierV8._json_text(None, str(raw)))
