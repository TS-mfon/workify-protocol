...
class WorkVerifierV5(gl.Contract):
    operator: Address
    work_type: str
    policy_version: str
    ...
    def verify(
        ...
        WorkVerifierV5._validate_request(None,
        ...
        def specification_fn():
            return WorkVerifierV5._load_specification_text(None, specification_url, specification_hash)

        specification_text = gl.eq_principle.strict_eq(specification_fn)
        specification = json.loads(specification_text)

        def evidence_fn():
            return WorkVerifierV5._build_input(None,
        ...
        parsed = WorkVerifierV5._normalize(None, raw_verdict, specification, policy_version, specification_hash, evidence_hash, attempt)
        verdict = json.dumps(parsed, sort_keys=True, separators=(",", ":"))
        key = WorkVerifierV5._key(None, job_id, attempt, appeal)
    ...
    def get_verdict(self, job_id: str, attempt: u32, appeal: bool) -> str:
        return self.verdicts.get(WorkVerifierV5._key(None, job_id, attempt, appeal), "")
    ...
    def _load_specification_text(self, specification_url: str, specification_hash: str) -> str:
        specification_bytes = WorkVerifierV5._fetch(None, specification_url, MAX_DOCUMENT_BYTES)
    ...
    def _build_input(self, work_type: str, policy_version: str, job_id: str, specification: dict, evidence_url: str, evidence_hash: str, attempt: u32, appeal: bool, appeal_context_url: str) -> str:
        evidence_bytes = WorkVerifierV5._fetch(None, evidence_url, MAX_DOCUMENT_BYTES)
        ...
        sources = WorkVerifierV5._fetch_sources(None, evidence)
        appeal_context = ""
        if appeal:
            appeal_context = WorkVerifierV5._fetch(None, appeal_context_url, MAX_DOCUMENT_BYTES).decode("utf-8", errors="replace")
        return WorkVerifierV5._prompt(None, work_type, policy_version, job_id, specification, evidence, sources, attempt, appeal, appeal_context)
    ...
    def _fetch_sources(self, evidence: dict) -> list:
        ...
        for artifact in artifacts[:MAX_SOURCES]:
            ...
            if not url.startswith("https://"):
                continue
            body = WorkVerifierV5._fetch(None, url, MAX_SOURCE_BYTES)
...
ATTEMPT: {attempt}
APPEAL: {appeal}
DOMAIN RUBRIC: {WorkVerifierV5._rubric(None, work_type)}
