...
class WorkVerifierV9(gl.Contract):
    operator: Address
    treasury: Address
    work_type: str
    policy_version: str
    ...
    def __init__(self, operator: str, treasury: str, work_type: str, policy_version: str):
...
Policy rubric: """ + WorkVerifierV9._rubric(None, work_type) + "\nADJUDICATION INPUT:\n" + adjudication_input
        ...
        parsed = WorkVerifierV9._normalize(None, raw_verdict, specification, policy_version, specification_hash, evidence_hash, attempt, appeal)
        verdict = json.dumps(parsed, sort_keys=True, separators=(",", ":"))
        key = WorkVerifierV9._key(None, job_id, attempt, appeal)
    ...
    def get_verdict(self, job_id: str, attempt: u32, appeal: bool) -> str:
        return self.verdicts.get(WorkVerifierV9._key(None, job_id, attempt, appeal), "")
    ...
    def _require_fee(self, treasury_address: str, job_id: str, attempt: u32, appeal: bool, fee_payer: str) -> None:
        payer = Address(fee_payer)
        if payer == Address("0x0000000000000000000000000000000000000000"):
            raise gl.vm.UserError("[EXPECTED] Invalid fee payer")
        key = job_id + (":appeal" if appeal else ":verification:" + str(attempt))
        payment = gl.get_contract_at(Address(treasury_address)).view().get_payment(key)
        amount = u256(payment.get("amount", u256(0)))
        if not bool(payment.get("funded", False)):
            raise gl.vm.UserError("[EXPECTED] Verification fee not finalized")
        if not payment.get("payer", "") or str(payment["payer"]).lower() != str(payer).lower():
            raise gl.vm.UserError("[EXPECTED] Verification fee payer mismatch")
    ...
    def _load_specification_text(self, specification_url: str, specification_hash: str) -> str:
        specification_bytes = WorkVerifierV9._fetch(None, specification_url, MAX_DOCUMENT_BYTES)
    ...
    def _build_input(self, work_type: str, policy_version: str, job_id: str, specification: dict, evidence_url: str, evidence_hash: str, attempt: u32, appeal: bool, appeal_context_url: str) -> str:
        evidence_bytes = WorkVerifierV9._fetch(None, evidence_url, MAX_DOCUMENT_BYTES)
        ...
        sources = WorkVerifierV9._fetch_sources(None, evidence)
        appeal_context = ""
        if appeal:
            appeal_context = WorkVerifierV9._fetch(None, appeal_context_url, MAX_DOCUMENT_BYTES).decode("utf-8", errors="replace")
        return WorkVerifierV9._prompt(None, work_type, policy_version, job_id, specification, sources, attempt, appeal, appeal_context)
    ...
    def _fetch_sources(self, evidence: dict) -> list:
        ...
        for artifact in artifacts[:MAX_SOURCES]:
            ...
            if not url.startswith("https://"):
                continue
            body = WorkVerifierV9._fetch(None, url, MAX_SOURCE_BYTES)
    ...
    def _normalize(self, raw, specification: dict, policy_version: str, specification_hash: str, evidence_hash: str, attempt: u32, appeal: bool) -> dict:
        value = raw if isinstance(raw, dict) else json.loads(WorkVerifierV9._json_text(None, str(raw)))
